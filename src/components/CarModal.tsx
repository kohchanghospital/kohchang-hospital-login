...
export type Vehicle = { id: number; registration_number: string };
            ...
            <div className="page-surface flex max-h-[calc(100vh-2rem)] w-full max-w-3xl flex-col overflow-hidden">
                ...
                <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
                    <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-5 py-5 sm:px-7">
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            <label className="space-y-1.5"><span className="text-sm font-semibold text-slate-700">พนักงานขับรถ <span className="text-red-500">*</span></span><select value={driverId} onChange={(e) => setDriverId(e.target.value)} className="w-full" disabled={readOnly || mastersLoading}><option value="">{mastersLoading ? "กำลังโหลด..." : drivers.length ? "เลือกพนักงานขับรถ" : "ไม่มีข้อมูลพนักงานขับรถ"}</option>{drivers.map((driver) => <option key={driver.id} value={driver.id}>{driver.name}</option>)}</select>{errorText(errors.driver_id)}</label>
                            <label className="space-y-1.5"><span className="text-sm font-semibold text-slate-700">ทะเบียนรถ <span className="text-red-500">*</span></span><select value={vehicleId} onChange={(e) => setVehicleId(e.target.value)} className="w-full" disabled={readOnly || mastersLoading}><option value="">{mastersLoading ? "กำลังโหลด..." : vehicles.length ? "เลือกทะเบียนรถ" : "ไม่มีข้อมูลทะเบียนรถ"}</option>{vehicles.map((vehicle) => <option key={vehicle.id} value={vehicle.id}>{vehicle.registration_number}</option>)}</select>{errorText(errors.vehicle_id)}</label>
                        </div>
                        <label className="block space-y-1.5"><span className="text-sm font-semibold text-slate-700">หัวข้อ <span className="text-red-500">*</span></span><input value={title} onChange={(e) => setTitle(e.target.value)} className="w-full" placeholder="หัวข้อการใช้รถยนต์" disabled={readOnly} />{errorText(errors.title)}</label>
                        <div className="rounded-2xl border border-slate-100 bg-slate-50/80 p-4">
                            <div className="mb-3 flex items-start justify-between gap-3"><div><h4 className="font-semibold text-slate-900">รายละเอียด</h4>{!readOnly && <p className="text-sm text-slate-500">เพิ่มรายละเอียดเป็นข้อ ๆ ระบบจะเก็บตามลำดับนี้</p>}</div>{!readOnly && <button type="button" onClick={() => setDetails((items) => [...items, ""])} className="icon-button bg-primary-600 text-white" aria-label="เพิ่มรายละเอียด"><Icons.PlusCircle /></button>}</div>
